...
local function update_status_for_file(file_path)
	...
	local diff_stats = vim.fn.system("git diff --numstat " .. vim.fn.shellescape(file_path))
	if vim.v.shell_error ~= 0 or diff_stats == "" then
		return ""
	end
	...
	if status.added > 0 then
		table.insert(status_txt, "+" .. status.added)
	end
